...
public class ThirdWarMap {
	...
	public static void addWalls() {
		...
		// right - shape centered
		for (int i = 8; i < 22; i++)
		{
			walls.add(new Corner(width-1, i));
		}
		//bottom - shape centered
		for (int i = 8; i < 22; i++)
		{
		...
		//Two 3x3 squares in the middle
		for (int i = 0; i < 4; i++)
		{
			walls.add(new Corner(i+8, 8));
			walls.add(new Corner(i+8, 9));
			walls.add(new Corner(i+8, 10));
			walls.add(new Corner(i+8, 11));
			
			walls.add(new Corner(i+17, 18));
			walls.add(new Corner(i+17, 19));
			walls.add(new Corner(i+17, 20));
			walls.add(new Corner(i+17, 21));
		}
	}
	...
	public static void tick(GraphicsContext gc) {
		...
		for (int i = 0; i < 4; i++)
		{
			gc.fillRect((i+8)*cornersize, 8*cornersize, (cornersize-2), (cornersize-2));
			gc.fillRect((i+8)*cornersize, 9*cornersize, (cornersize-2), (cornersize-2));
			gc.fillRect((i+8)*cornersize, 10*cornersize, (cornersize-2), (cornersize-2));
			gc.fillRect((i+8)*cornersize, 11*cornersize, (cornersize-2), (cornersize-2));
			
			gc.fillRect((i+17)*cornersize, 18*cornersize, (cornersize-2), (cornersize-2));
			gc.fillRect((i+17)*cornersize, 19*cornersize, (cornersize-2), (cornersize-2));
			gc.fillRect((i+17)*cornersize, 20*cornersize, (cornersize-2), (cornersize-2));
			gc.fillRect((i+17)*cornersize, 21*cornersize, (cornersize-2), (cornersize-2));
		}
		...
		if (score >= scoreRequirement)
		{
			...
			foodCount = 0;
			WarMode wm = new WarMode();
			wm.start(appController, speed, score);
			score = 0;
		}
